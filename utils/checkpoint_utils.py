...
def save_metrics_model(epoch, model, losses_accs, path_dict, save_period):
    ...
    np.savez_compressed(os.path.join(path_dict['cp_path'], 'losses_accs'),
                        ...
                        train_accuracies=losses_accs['train_accuracies'],
                        validation_losses=losses_accs['validation_losses'],
                        validation_accuracies=losses_accs['validation_accuracies'])
...
def metric_visualizer(losses_accs, save_path):
    ...
    ax2.plot(epoch_range, losses_accs['train_accuracies'], color='tab:orange',
             linestyle=':', linewidth=2, label='Train Accuracy')
    ax2.plot(epoch_range, losses_accs['validation_accuracies'], color='tab:orange',
    ...
    ax_yticks_m, ax_yticks_M = ax_yticks, ax_yticks[-1]
    ...
    for spine_loc, spine in ax.spines.items():
        if spine_loc in ['right', 'top']: spine.set_visible(False)
        if spine_loc in ['left']: spine.set_color('tab:blue')

    for spine_loc, spine in ax2.spines.items():
        if spine_loc in ['left', 'top']: spine.set_visible(False)
        if spine_loc in ['right']: spine.set_color('tab:orange')
    ...
    ax.set_xlim(left=0, right=len(losses_accs['train_losses'])+1)
    ax2.set_xlim(left=0, right=len(losses_accs['train_losses'])+1)
